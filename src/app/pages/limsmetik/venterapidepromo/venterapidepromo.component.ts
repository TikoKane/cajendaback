...
export class VenterapidepromoComponent implements OnInit {
  ...
  onLogin(f: NgForm) {
    ...
    this.serviceVente.insertintoAjoutProduitPromo(this.contenue).subscribe(resp=>{ if(resp['succes']==false){this.bad(resp['message']);}else{this.good(resp['message']);}this.reloadComponent();this.valider=true;},error1 => {this. baddd()});
    this.serviceAchat.getTotalMontantAchete().subscribe(data=>{this.montant=data['totalMontant'][0].total},error1 => {console.log(error1);});
